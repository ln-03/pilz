...
let offlinePhotoDownloadStop=false;
async function offlinePhotoCountForTaxon(id){return (await idbGetPhotos(String(id),'inat')).length}
async function offlinePhotoStatus(){
  const species=[...byId.values()].filter(n=>n.rank==='species');
  let taxaDone=0, photos=0;
  for(const n of species){const c=await offlinePhotoCountForTaxon(n.id);photos+=c;if(c>=4)taxaDone++}
  const el=$('#offlinePhotosState'); if(el)el.textContent=`${taxaDone} von ${species.length} Arten vollständig · ${photos} Bilder lokal gespeichert`;
  const p=$('#offlinePhotosProgress'); if(p){p.max=Math.max(1,species.length);p.value=taxaDone}
  return {species,taxaDone,photos};
}
function openOfflinePhotos(){offlinePhotoDownloadStop=false;$('#offlinePhotosPanel').classList.remove('hidden');offlinePhotoStatus()}
async function fetchAndStoreInatPhotos(n,need){
  const obs=await fetchJSON(`${INAT.base}/observations?place_id=${INAT.placeId}&taxon_id=${encodeURIComponent(n.id)}&photos=true&per_page=20&order_by=votes&order=desc`);
  const urls=[];
  for(const o of (obs.results||[])){for(const p of (o.photos||[])){let u=p.url||'';if(u){u=u.replace('square','medium');if(!urls.includes(u))urls.push(u)}if(urls.length>=need)break}if(urls.length>=need)break}
  let saved=0;
  for(const u of urls.slice(0,need)){try{const r=await fetch(u,{mode:'cors'});if(!r.ok)continue;const blob=await r.blob();await idbAddPhoto(String(n.id),new File([blob],`inat_${n.id}_${Date.now()}_${saved+1}.jpg`,{type:blob.type||'image/jpeg'}),'inat');saved++}catch(e){console.warn('Foto konnte nicht gespeichert werden',n.name,u,e)}}
  return saved;
}
async function downloadMissingOfflinePhotos(){
  if(!navigator.onLine)return alert('Zum Herunterladen der Bilder brauchst du Internet.');
  const btn=$('#downloadOfflinePhotos'), state=$('#offlinePhotosState'), prog=$('#offlinePhotosProgress');
  btn.disabled=true; offlinePhotoDownloadStop=false;
  const species=[...byId.values()].filter(n=>n.rank==='species'); prog.max=Math.max(1,species.length);
  let complete=0, totalPhotos=0, failures=0;
  try{
    for(let i=0;i<species.length;i++){
      const n=species[i]; let c=await offlinePhotoCountForTaxon(n.id);
      if(c<4){
        state.textContent=`${i+1}/${species.length}: ${n.name} · ${c}/4 Bilder …`;
        try{await fetchAndStoreInatPhotos(n,4-c)}catch(e){failures++;console.warn(e)}
        c=await offlinePhotoCountForTaxon(n.id);
      }
      totalPhotos+=c;if(c>=4)complete++;prog.value=i+1;
      if(i%8===7)await new Promise(r=>setTimeout(r,250));
    }
    state.textContent=`Fertig: ${complete} von ${species.length} Arten vollständig · ${totalPhotos} Bilder lokal${failures?` · ${failures} Abfragen fehlgeschlagen`:''}.`;
...
$('#offlinePhotosBtn')?.addEventListener('click',openOfflinePhotos);
$('#checkOfflinePhotos')?.addEventListener('click',offlinePhotoStatus);
$('#downloadOfflinePhotos')?.addEventListener('click',downloadMissingOfflinePhotos);
